...
import { requireAdminProfile, requireParentProfile, requirePlatformProfile } from "@/features/auth/server";
...
export async function sendReminderAction(formData: FormData) {
  ...
  const { data: family, error: familyError } = await supabase
    ...
    .single();
  if (familyError || !family) throw new Error(familyError?.message ?? "Family not found.");
  const providerResponse = await sendNotification(parsed.channel, {
    familyId: family.id,
