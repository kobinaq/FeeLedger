...
export default async function ParentReceiptsPage() {
  ...
  return (
    <>
      ...
      <div className="mt-5 space-y-3">{family.receipts?.map((receipt: any) => <div key={receipt.id} className="space-y-2"><ReceiptCard receipt={receipt} /><Button variant="secondary">Download Receipt</Button></div>)}</div>
    </>
  );
}
