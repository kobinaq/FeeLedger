...
export default async function ParentBillsPage() {
  ...
  const byChild = new Map<string, NonNullable<typeof family.bills>>();
  for (const bill of family.bills ?? []) {
    const key = bill.student_id ?? "family";
  ...
  return (
    <>
      ...
      <div className="mt-5 space-y-6">
        {[...byChild.entries()].map(([studentId, bills]) => {
          const student = family.students?.find((item) => item.id === studentId);
          const heading = student ? `${student.first_name} ${student.last_name}` : "Family account";
          return (
            <section key={studentId}>
              <h2 className="mb-3 text-lg font-semibold text-slate-900">{heading}</h2>
                    ...
                    <Card key={bill.id}>
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-500">Due {shortDate(bill.due_date)}</p>
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <strong>{money(bill.paid_amount)}</strong>
                        </div>
                        <div>
                      ...
                      <PayNowForm amountDue={left} billId={bill.id} studentId={bill.student_id} compact />
                    </Card>
                  );
                })}
              </div>
