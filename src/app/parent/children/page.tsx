      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {family.students?.map((student) => (
          <Card key={student.id}>
            ...
            <p className="mt-2 text-sm text-slate-500">{student.student_code}</p>
            <p className="mt-3 text-sm font-medium text-slate-800">{student.classes?.name ?? "Class not linked"}</p>
            <p className="mt-3 text-sm">Services: {student.optional_services.join(", ") || "None"}</p>
          </Card>
        ))}
      </div>
    </>
