    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        <Card>
          ...
          <Table><thead><tr><Th>Receipt</Th><Th>Date</Th><Th>Family</Th><Th>Student</Th><Th>Method</Th><Th>Amount</Th><Th></Th></tr></thead><tbody>{receipts.map((receipt) => <tr key={receipt.id}><Td><Link className="font-semibold text-brand-green" href={`/admin/receipts/${receipt.id}`}>{receipt.receipt_number}</Link></Td><Td>{shortDate(receipt.receipt_date)}</Td><Td>{receipt.families?.guardian_full_name}</Td><Td>{receipt.students?.first_name}</Td><Td>{receipt.method}</Td><Td>{money(receipt.amount)}</Td><Td><Button variant="secondary">Resend</Button></Td></tr>)}</tbody></Table>
        </Card>
